 ...
 *
 * <p>This advice intercepts every response body before serialisation. Responses that are already an
 * {@link ApiResponse} or an RFC 9457 {@link ProblemDetail} are passed through untouched. Raw {@code
 * String} and {@code byte[]} returns, as well as Spring {@code Resource} streams, are also excluded
 * to avoid corrupting plain-text or binary payloads.
 *
 * <p>The current request URI is read from the injected {@link HttpServletRequest} (request-scoped
 * proxy) so the path is always accurate even in async contexts.
 */
...
public class ApiResponseAdvice implements ResponseBodyAdvice<Object> {
   ...
   *
   * <p>Returns {@code false} for {@code String}, {@code byte[]}, and any subtype of {@link
   * org.springframework.core.io.Resource} so that raw and binary responses are never wrapped.
   *
  ...
  /**
   * Wraps the body in an {@link ApiResponse} unless it is already an {@link ApiResponse} or a
   * {@link ProblemDetail}.
   *
   ...
   * @param request the server-side HTTP request (unused; URI is read from the injected servlet
   *     request)
  ...
  public Object beforeBodyWrite(
    ...
    return ApiResponse.success(body, "OK", this.request.getRequestURI());
  }
}
