...
/**
 * Exception handler that intercepts errors from Feign clients and converts them to the API
 * standard.
 *
 * <p>Only activated if the project has Feign in the classpath.
 */
...
public class FeignExceptionHandler {
  private static final org.slf4j.Logger log =
      org.slf4j.LoggerFactory.getLogger(FeignExceptionHandler.class);
  private final ApiStandardProperties properties;
  private final org.springframework.context.MessageSource messageSource;

  private static final com.fasterxml.jackson.core.type.TypeReference<java.util.Map<String, Object>>
      MAP_TYPE = new com.fasterxml.jackson.core.type.TypeReference<>() {};

  public FeignExceptionHandler(
      ApiStandardProperties properties, org.springframework.context.MessageSource messageSource) {
    this.properties = properties;
    this.messageSource = messageSource;
  }

  /**
   * Handles exceptions from Feign clients and maps them to a local error code and status.
   *
  ...
  public ProblemDetail handleFeignException(
      feign.FeignException ex, HttpServletRequest request, java.util.Locale locale) {
  ...
  private HttpStatus resolveHttpStatus(int externalStatus) {
    HttpStatus status =
        (externalStatus >= 400 && externalStatus < 500)
            ? HttpStatus.resolve(externalStatus)
  ...
  private String resolveInitialDetail(
      HttpStatus status, feign.FeignException ex, java.util.Locale locale) {
