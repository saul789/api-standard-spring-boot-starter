...
/**
 * Global exception handler that centralizes all error responses in RFC 9457
 * format (Problem Detail).
 *
 * <p>
 * It provides automatic enrichment of responses with machine-readable error
 * codes,
...
public class GlobalExceptionHandler {
    ...
    public GlobalExceptionHandler(MessageSource messageSource, ProblemDetailService problemDetailService) {
        this.messageSource = messageSource;
        this.problemDetailService = problemDetailService;
    }

    /**
     * Handles problem-based exceptions (ProblemException and subclasses like
     * BusinessException).
     */
    @ExceptionHandler(ProblemException.class)
    public ProblemDetail handleProblemException(ProblemException ex, HttpServletRequest request, Locale locale) {
    ...
    public ProblemDetail handleValidationException(MethodArgumentNotValidException ex, HttpServletRequest request, Locale locale) {
    ...
    public ProblemDetail handleConstraintViolation(jakarta.validation.ConstraintViolationException ex,
        ...
        List<ValidationError> errors = ex.getConstraintViolations().stream()
                .map(v -> {
                    ...
                    String message = (template != null && template.startsWith("{") && template.endsWith("}"))
                            ? messageSource.getMessage(template.substring(1, template.length() - 1), null, v.getMessage(), locale)
                            : v.getMessage();
    ...
    public ProblemDetail handleNoResourceFoundException(Exception ex, HttpServletRequest request, Locale locale) {
        return problemDetailService.createProblem(
    ...
    public ProblemDetail handleMethodNotAllowed(HttpRequestMethodNotSupportedException ex, HttpServletRequest request, Locale locale) {
        return problemDetailService.createProblem(
    ...
    public ProblemDetail handleMediaTypeNotSupported(Exception ex, HttpServletRequest request, Locale locale) {
        return problemDetailService.createProblem(
    ...
    public ProblemDetail handleResponseStatusException(ResponseStatusException ex, HttpServletRequest request, Locale locale) {
        return problemDetailService.createProblem(
    ...
    public ProblemDetail handleGenericException(Exception ex, HttpServletRequest request, Locale locale) {
