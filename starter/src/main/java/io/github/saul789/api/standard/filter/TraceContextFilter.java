...
/**
 * Servlet filter that establishes a W3C Trace Context ({@code traceparent})
 * for every incoming request.
 *
 * <p>If the request already carries a valid {@code traceparent} header the
 * existing trace-id is reused; otherwise a new one is generated. The resolved
 * trace-id is stored in {@link MDC} under the key {@code "traceId"} so it is
 * available to all loggers and to exception handlers that embed it in
 * RFC 9457 responses.
 *
 * <p>The MDC context is <em>always</em> cleared in a {@code finally} block to
 * prevent trace-id leakage across requests on reused threads.
 *
...
public class TraceContextFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        String traceId = isValidTraceparent(traceparentHeader)
     ...
     *
     * <p>Uses version {@code 00}, a newly generated 16-character span-id, and
     * sampled flag {@code 01}.
     *
     ...
     *
     * <p>A valid value must have exactly four dash-separated parts where the
     * second part (trace-id) is 32 lowercase hex characters and the third part
     * (parent-id) is 16 lowercase hex characters.
     *
