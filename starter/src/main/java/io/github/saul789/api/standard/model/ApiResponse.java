 ...
 *
 * <p>Wraps the actual payload with metadata (timestamp, trace-id, path) to provide a consistent
 * structure across all endpoints. Errors are communicated via RFC 9457 {@code ProblemDetail}
 * instead.
 *
 * <p>Use the factory methods {@link #success} and {@link #error} to construct instances; the
...
public final class ApiResponse<T> {
  ...
  private ApiResponse(boolean success, String message, T data, String path, String traceId) {
  ...
  /**
   * Returns the W3C Trace Context trace-id propagated via MDC, or {@code null} if no tracing
   * context was active.
   */
