...
public class ApiStandardOpenApiCustomizer implements OpenApiCustomizer {
    ...
    private void addErrorResponse(ApiResponses responses, String statusCode, String description) {
        if (!responses.containsKey(statusCode)) {
            ...
            problemSchema.addProperty("type", new StringSchema()
                    .description("URI reference identifying the problem type").example("urn:problem-type:error"));
            problemSchema.addProperty("title",
                    new StringSchema().description("Short, human-readable problem title").example(description));
            problemSchema.addProperty("status",
                    new IntegerSchema().description("HTTP status code").example(Integer.parseInt(statusCode)));
            problemSchema.addProperty("detail", new StringSchema().description("Human-readable explanation")
                    .example("Detailed message about the error..."));
            problemSchema.addProperty("instance",
                    new StringSchema().description("URI reference of the specific occurrence"));
            problemSchema.addProperty("code",
                    new StringSchema().description("Machine-readable error code").example("ERROR_CODE"));
            problemSchema.addProperty("traceId",
                    new StringSchema().description("Unique request trace identifier").example("uuid-1234-5678"));
