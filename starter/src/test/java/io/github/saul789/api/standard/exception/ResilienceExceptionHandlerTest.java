...
class ResilienceExceptionHandlerTest {
  ...
  void setUp() {
    ...
    ProblemDetailService problemDetailService =
        new ProblemDetailService(messageSource, new ApiStandardProperties(), new ArrayList<>());

    ResilienceExceptionHandler handler = new ResilienceExceptionHandler(problemDetailService);

    this.mockMvc =
        MockMvcBuilders.standaloneSetup(new DummyController()).setControllerAdvice(handler).build();
  }

  @Test
  void shouldHandleCallNotPermittedException() throws Exception {
    mockMvc
        .perform(get("/test/circuit-breaker"))
